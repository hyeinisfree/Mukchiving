...
const userPost = (req, res) => {
  ...
  const userPost = postService.userPost(id, async function(err, results) {
    if(results[0]) {
      ...
      const getUserPostImages = await postService.getUserPostImages(post_id, function(err, results){
        if(results) {
          user_data.post_images = results;
          return res.json({success: true, message:"피드 조회 성공", user_data: user_data});
        } else return res.json({success: true, message:"피드 조회 성공", user_data: user_data});
      })
